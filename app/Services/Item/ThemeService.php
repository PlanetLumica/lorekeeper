...
class ThemeService extends Service {
    ...
    public function updateData($tag, $data) {
        ...
        try {
            ...
            if (!isset($data['rewardable_id']) && !isset($data['all_themes'])) {
                return true;
            }
            if (isset($data['all_themes'])) {
                $assets = ['all_themes' => 1];
            } else {
    ...
    public function act($stacks, $user, $data) {
        ...
        try {
            ...
            if (!count($options)) {
                throw new \Exception('There are no more options for this theme redemption item.');
            }
            if (count($options) < array_sum($data['quantities'])) {
                throw new \Exception('You have selected a quantity too high for the quantity of themes you can unlock with this item.');
            }
